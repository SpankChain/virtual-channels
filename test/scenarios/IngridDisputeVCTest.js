...
contract('Test Ingrid Disputed VC Payments', function(accounts) {
  ...
  it("Alice creates lc state lcS1 containing vcSO with Ingrid", async () => {
    ...
    vcRootHash_AI = Utils.bufferToHex(merkle.getRoot())
    ...
    AI_lcS1 = web3latest.utils.soliditySha3(
      ...
      { type: 'bytes32', value: vcRootHash_AI }, // VC root hash
      { type: 'address', value: partyA }, // partyA
      { type: 'address', value: partyI }, // hub
  ...
  it("Bob creates lc state lcS1 containing vcSO with Ingrid", async () => {
    ...
    vcRootHash_BI = Utils.bufferToHex(merkle.getRoot())
    ...
    BI_lcS1 = web3latest.utils.soliditySha3(
      ...
      { type: 'uint256', value: '1' }, // sequence
      { type: 'uint256', value: '1' }, // open VCs
      { type: 'bytes32', value: vcRootHash_BI }, // VC root hash
      { type: 'address', value: partyB }, // partyA
      { type: 'address', value: partyI }, // hub
  ...
  it("Ingrid initiates settling on-chain with byzantine Alice and Bob", async () => {
    await lc.updateLCstate(web3latest.utils.sha3('1111', {encoding: 'hex'}), ['2', '3', web3latest.utils.toWei('4'), web3latest.utils.toWei('6')], vcRootHash_AI, AI_lcS2_sigA, AI_lcS2_sigI)
    await lc.updateLCstate(web3latest.utils.sha3('2222', {encoding: 'hex'}), ['1', '1', web3latest.utils.toWei('1'), web3latest.utils.toWei('17')], vcRootHash_BI, BI_lcS1_sigB, BI_lcS1_sigI)
  ...
  it("Ingrid initiates settling vc with initial state (AI channel)", async () => {
    var buf = Utils.hexToBuffer(AB_vc1)
    var buf1 = Utils.hexToBuffer(AB_vc2)
    var buf2 = Utils.hexToBuffer(AB_vcS0)
    var elems = []
