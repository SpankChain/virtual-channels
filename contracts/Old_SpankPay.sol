...
contract SpankPay {
    ...
    // using totals should make it impossible to empty an account and leave value in

    struct Account {
        uint256 ethBalances[3]; // hub / user / total
        uint256 tokenBalances[3] // hub / user / total
        uint256 txCount; // persisted onchain even when empty
        address personWhoStartedExit; // TODO better name
        uint256 accountClosingTime;
        uint256 tabClosingTime;
        bytes32 tabRoot;
        uint256 tabCount;
        Status status;
        mapping(address => mapping(address => Tab)) tabs; // [sender, receiver]
    }

    // When users open tabs - receiver is performer
    // tabs[user][receiver]

    // When hub opens with user - user is performer
    // tabs[sender][user]

    // Account | Sender | Receiver
    // ameen   | ameen  | wolever
    // wolever | ameen  | wolever
    ...
    // perf    | fred   | perf
    // fred    | fred   | perf
    // ^ these two can be settled independently

    // primary key for tabs = user address + sender address + receiver address
    ...
    function userOpenAccount(
        ...
        require(reserveETH >= hubETH), "insufficient ETH");
    ...
    // Hub deposit into channel
    // 1. Hub requests sig from performer acknowledging "pendingETH" and "pendingTokens"
    // 2. Hub calls hubDepositIntoAccount w/ sigs
    // 3.
    ...
    //  3. WAIT
    //  4. hub deposits 49 BOOTY (up to BOOTY limit)
    //  5. WAIT (can still tip up to 20 BOOTY)
    //  6. now tip up to 69 BOOTY -> UX could be that BOOTY limit increases after some time
    //  - This would require timer
    ...
    function hubAuthorizedEmptyAccount(
        ...
        // eth balances must be conserved
        // TODO - this is broken
        // 0. userETH = 100, hubETH = 100
        // 1. mutually signed deposit w/ money 100 total in tabs
        //  - account.userETH = 50, account.hubETH = 50
    ...
    // 1. deposit ETH into performer channel hub calls hubDepositIntoAccount
    // 2. WAIT
    ...
    // state2 { hubETH: 0, hubBOOTY: 210, userETH: 0, userBOOTY: 0, pendingHubETHDeposit: 200, pendingUserETHWithdrawal: 200 }
    // state3 { hubETH: 0, hubBOOTY: 210, userETH: 0, userBOOTY: 0 }
    ...
    // offchain protocol
    // 1. performer requests withdrawal of 200 BOOTY (in ETH, to target address)
    // 2. hub provides a signed state update with (only hub can call, so it doesn't matter):
    //  - pending deposit of ETH for value of the BOOTY
    //  - pending ETH withdrawal from the performer
    //  - timeout (to protect performer against exchange rate fluctuations)
    //  - can optionally include a hub BOOTY withdrawal

    // TODO
    // - if we did the above, is there any difference between opening a new account or depositing into one?
    // - we check that all tabs are closed
    // - key factor: is the account in a state where more money can be deposited?
    // - for user, yes -> can combine userOpenAccount + userDepositInto
    // - still want hubOpenAccount -> airdrop
    // - Key Question: Does the hub ever want to open account/deposit funds for a user who is NOT online?
    //   - probably not, because we would need to know the private keys
    //   - this means it's probably fine to combine into a single function...
    // - COMBINE ALL THE THINGS
    //   - SUPER ULTRA MEGA CHECKPOINT
    //   - authorizedUpdate -> hubOnly -> timeout -> pendingDeposits (user/hub, eth/booty), pendingWithdrawals (user/hub, eth/booty)
    ...
    // How and when does hub claim its money?
    // - user deposits $100 ETH x3 and tips it all
    // -
    ...
    function startExit // start exit w/ onchain state
